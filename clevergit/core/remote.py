...
def add_remote(repo_path: Path, name: str, url: str) -> None:
    ...
    try:
        result = client.run_command(['remote', 'add', name, url])
        if not result.success:
            raise RemoteError(f"Failed to add remote: {result.error}")
    except Exception as e:
...
def remove_remote(repo_path: Path, name: str) -> None:
    ...
    try:
        result = client.run_command(['remote', 'remove', name])
        if not result.success:
            raise RemoteError(f"Failed to remove remote: {result.error}")
    except Exception as e:
...
def rename_remote(repo_path: Path, old_name: str, new_name: str) -> None:
    ...
    try:
        result = client.run_command(['remote', 'rename', old_name, new_name])
        if not result.success:
            raise RemoteError(f"Failed to rename remote: {result.error}")
    except Exception as e:
...
def set_remote_url(repo_path: Path, name: str, url: str) -> None:
    ...
    try:
        result = client.run_command(['remote', 'set-url', name, url])
        if not result.success:
            raise RemoteError(f"Failed to set remote URL: {result.error}")
    except Exception as e:
...
def get_remote_url(repo_path: Path, name: str = "origin") -> Optional[str]:
    ...
    try:
        result = client.run_command(['remote', 'get-url', name])
        if result.success and result.output:
...
def show_remote(repo_path: Path, name: str = "origin") -> Dict[str, any]:
    ...
    try:
        result = client.run_command(['remote', 'show', name])
        if not result.success:
            raise RemoteError(f"Failed to show remote: {result.error}")
